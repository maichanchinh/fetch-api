...
```
var FetchApi = require('fetch-api-warpper');
var api = new FetchApi('/v1');

api.get('users', {name: 'foo'}).then(function(response) {});
// GET request on '/v1/users?name=foo'

api.post('posts', {title: 'foo', content: 'bar'}).then(function(response) {});
// POST request on '/v1/posts' with data {"title": "foo", "content": "bar"}
```
...
var api = new FetchApi('/', addOptions, useTrailingSlashes);
```
